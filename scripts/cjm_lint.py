...
DISCLAIMER_BLOCK_MARKERS = (
    ...
    # Раздел «Границы этого отчёта (конкурентная отстройка)» зафиксирован статично
    # ПРЯМО в competitive_report_template.md (не в disclaimers.md — см. комментарий
    # в начале того файла) и по правилу SKILL.md §3 копируется в comp_report.md
    # ДОСЛОВНО. Он закономерно и правомерно упоминает «доля рынка»/«switch rate» —
    # но ОБЪЯСНЯЯ, что они НЕ оцениваются этим отчётом (дисклеймер, а не обещание),
    # ровно та же природа ложного срабатывания, что уже чинилась для
    # DISCLAIMER_BLOCK_CJM выше. Тот же принцип: сам текст легенды/маркеров 🟢🟡🔴
    # раздела 1 comp_report.md лежит ДО этого маркера, вне маскируемого блока — не
    # теряется для правила 3а.
...
# Расширяемый список (label, regex) — минимум по заданию [B2] п.1(а). Добавлять
# новые ИИ-измы — просто дописать кортеж сюда, ничего в check_ai_isms менять не нужно.
...
def check_ai_isms(lines: list[str]) -> list[StyleWarning]:
    ...
    позиции символов в строке — если диапазон уже пойман именным паттерном
    AI_ISM_PATTERNS, общая проверка длины дефисного слова НЕ добавляет второе
    предупреждение на тот же спан (см. докстринг модуля).
    """
    ...
    for i, line in enumerate(lines):
        ...
        for m in HYPHENATED_WORD_RE.finditer(line):
            ...
            if any(span[0] < e and s < span[1] for s, e in matched_spans):
                continue  # уже отмечено именным паттерном выше — не дублируем
            warnings.append(
                StyleWarning(
                    line=i + 1,
                    message=(
