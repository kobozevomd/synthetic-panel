...
=== Схема входа: 01_segments_merged.yaml (моё рабочее допущение о формате —
    ...
    segments:                            # ЯДРО — сегменты со stability "3/3"/"2/3"
      - id: molodye_mamy_posle_rodov     # str, snake_case, латиница — как в panel/segments/*.yaml (v1 §9)
        name: "Молодые мамы после родов"     # ЧЕЛОВЕЧЕСКОЕ имя (после стадии «Редактор-гуманизатор», v1.2 §Модуль 1 п.2)
        name_original: "Молодые мамы в постнатальном периоде"  # НОВОЕ, v1.2 — опционально;
                                              # исходное (до правки редактором) имя, для трассируемости
        ...
        persona_jitter:
          age: [22, 34]
          gender: ["ж"]                              # НОВОЕ, v1.2 §Модуль 2 п.1 — опционально; НЕ закрытый
                                                       # словарь (в отличие от двух полей ниже), см.
                                                       # check_persona_jitter_vocab; отсутствует у старых
                                                       # сегментов -> targeting_export.py честно пишет
                                                       # «уточнить», а не восстанавливает задним числом
...
только generate.py/report.py — см. их код; generate.py на момент этой итерации
ещё НЕ читает `persona_jitter.gender` при сборке персона-строки — потребитель
этого поля в v1.2 это targeting_export.py, не сама SSR-генерация ответов).

=== Схема выхода: panel/segments/<category_slug>/<id>.yaml (v1 §9 + доп. поля) ===

    id, name, mode: persona                    # как в v1 §9
    name_original                               # НОВОЕ, v1.2 §Модуль 1 п.2 — только если было во входе
    stability, source, trust                    # НОВЫЕ поля этой итерации (v1.1 §3)
    description, axes, behavior, language,
    persona_jitter, brands_context               # проброшены из входа как есть (если были); gender
                                                   # внутри persona_jitter передаётся тем же путём —
                                                   # ОТДЕЛЬНОГО кода для него не нужно (см. build_segment_yaml)
...
def check_persona_jitter_vocab(segment: dict) -> list[str]:
    ...
    # v1.2 §Модуль 2 п.1 (задание [B2] п.4 "passthrough gender"): gender — НЕ
    # закрытый словарь (в отличие от income_level/city_tier выше) — примеры
    # спецификации ["ж","м"] и ["преимущественно ж"] показывают произвольные
    # описательные строки, не фиксированный enum, поэтому здесь только
    # проверка ФОРМЫ (должен быть список строк), не проверка ПРИНАДЛЕЖНОСТИ
    # словарю. Само значение пробрасывается насквозь без изменений — см.
    # build_segment_yaml, persona_jitter целиком уже в V1_PASSTHROUGH_FIELDS,
    # gender внутри него передаётся автоматически, без отдельного кода.
    gender = jitter.get("gender")
    if gender is not None and not (isinstance(gender, list) and all(isinstance(g, str) for g in gender)):
        warnings.append(
            f"сегмент {segment.get('id')!r}: persona_jitter.gender должен быть списком строк "
            f"(например ['ж','м'] или ['преимущественно ж'], spec_synthetic-panel_v1.2.md §Модуль 2 "
            f"п.1), получено: {gender!r} — передано насквозь как есть, но targeting_export.py и "
            f"дальнейшие потребители этого поля могут прочитать его некорректно."
        )
    return warnings
...
def build_segment_yaml(segment: dict, unstable: bool = False) -> dict:
    ...
    # v1.2 §Модуль 1 п.2 (задание [B2] п.4 "passthrough name_original"):
    # трассируемость после стадии «Редактор-гуманизатор» — `name` становится
    # человеческим именем (переписанным по правилам нейминга §1), а исходное
    # (до правки) имя сохраняется рядом в `name_original`, если оно было
    # передано во входной записи 01_segments_merged.yaml. НЕ обязательное поле
    # (старые прогоны/сегменты без этой стадии его не несут — тогда просто
    # отсутствует в выходном YAML, как и раньше); НЕ синтезируется задним
    # числом из `name`, если отсутствует во входе (не выдумываем историю,
    # которой не было).
    if segment.get("name_original"):
        out["name_original"] = segment["name_original"]
    out.update(
        {
